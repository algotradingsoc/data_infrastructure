...
def create_database_id(db_name,data_directory):
    ...
    for name in file_names:
        ...
        for row in csv_file:
            ...
            try:
                collection.insert_one(row)
                try:
                    id_ticker_HashMap[row["finnhub_id"]][(row["symbol"],row["class"])].append(dt)
                except:
                    id_ticker_HashMap[row["finnhub_id"]] = {(row["symbol"],row["class"]):[dt]}
            except:
    ...
    for finnhub_id, ticker_class_dates in id_ticker_HashMap.items():
        for ticker_class, dates in ticker_class_dates.items():
            ticker_id_HashMap[(ticker_class[0],ticker_class[1],dates[0],dates[-1])] = finnhub_id
    
    ticker_id_meta_data = db["ticker_id_meta_data"]
    ticker_id_meta_data.create_index([("ticker",pymongo.ASCENDING),("class",pymongo.ASCENDING),("start",pymongo.ASCENDING),("end",pymongo.ASCENDING)],unique=True)
    for key,value in ticker_id_HashMap.items():
        row = {"ticker":key[0],"class":key[1],"start":key[2],"end":key[3],"finnhub_id":value}
...
if __name__ == "__main__":
    ...
    create_database_id("kaggle_db","kaggle_data")
